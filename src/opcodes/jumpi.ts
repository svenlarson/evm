...
export default (opcode: Opcode, state: EVM): void => {
    ...
    if (!BigNumber.isInstance(jumpLocation)) {
        // target location is not a number
        state.halted = true;
    ...
    if (!jumpLocationData || jumpLocationData.name !== 'JUMPDEST') {
        ...
        // target location doesn't exist or is not a JUMPDEST
        state.instructions.push(new REQUIRE(jumpCondition));
        return;
    }
    const jumpIndex = opcodes.indexOf(jumpLocationData);

    if (BigNumber.isInstance(jumpCondition)) {
        // the jump condition is a simple number
        if (
            jumpIndex >= 0 &&
            !jumpCondition.equals(0) &&
            !(opcode.pc + ':' + jumpLocation.toJSNumber() in state.jumps)
        ) {
            // the jump condition is true, and we didn't jump before, so jump
        ...
        // we didn't jump before and it's a SIG condition
        console.log('JUMPI SIG');
        if (jumpIndex >= 0) {
            ...
            if (
                ...
                if (
                    ...
                    // function type 2.1 : new variable
                    const fullFunction = (functionHashes as any)[jumpCondition.hash];
                    state.variables[
                    ...
                    // function type 2.2 : known variable
                    const fullFunction = (functionHashes as any)[jumpCondition.hash];
                    state.variables[
                        state.functions[jumpCondition.hash].items[0].items[0].location
        ...
        console.log('JUMPI CALLDATASIZE');
        // we didn't jumpb before, and the condition is a
        // CALLDATASIZE < 4
        // CALLDATASIZE == 0

        // this happens really at the start of a contract; the
        // JUMPI goes to the fallback function
        if (jumpIndex >= 0) {
            state.halted = true;
        ...
        // we didn't jump before
        console.log('REGULAR JUMPI');
