...
export default (opcode: Opcode, state: EVM): void => {
    ...
    if (storeLocation.name === 'SHA3') {
        ...
        const mappingLocation = mappingItems.find((mappingItem: any) =>
            LOCAL_VARIABLE.isInstance(mappingItem)
        );
        const mappingParts = mappingItems.filter(
            (mappingItem: any) => !LOCAL_VARIABLE.isInstance(mappingItem)
        );
        ...
        LOCAL_VARIABLE.isInstance(storeLocation.right)
    ) {
        ...
        storeLocation.name === 'ADD' &&
        LOCAL_VARIABLE.isInstance(storeLocation.left) &&
        ...
        false &&
        LOCAL_VARIABLE.isInstance(storeLocation) &&
